...
| :-------- | :--------| :-- |:--------|--------:| --------:|
| access_token  | 服务商账户Token | 服务商账户授权Token | string  | 是 | 无 |
| status  | 采购单状态| 0待发货 1已发货 2已关闭 3全部 | integer  | 否 | 3 |
| limit  | 限制大小 | 指定返回记录的数量，系统会做验证 | integer | 否 | 100 |
| offset  | 偏移量 | 指定返回记录的开始位置  | integer  | 否 | 0 |
...
| administrator | BD管理员 | 仅适用后台操作 | integereger | 是 | 无 |
...
| 参数名      |含义   | 规则说明  | 参数类型 | 是否必须 | 缺省值 |
| :-------- | :--------| :-- |:--------|--------:| --------:|
| order_no  | 采购单号 | 20位的随机16进制格式  | string  | 是 | 无 |


**请求实例**：

```http
GET /v1/transaction?order_no=55c9c60565884 HTTP/1.1
Host: api.weipei.cc
Authorization: 56257a57e2a48adcb77beb724217dde2535371a4
```
...
```json
{
  ...
  "error_code": 0,
  "error_msg": "",
  "refound_no":"201509302135040000321",
}
```

| 返回参数 |含义| 参数类型 | 长度 |
|:--|:--|--:|--:|
|refound_no|退款单号|32位16进制格式| 32|
...
{
	"order_no": "55816e7c2bc96",
	"tranding_no": "201509301035040000001",
	"channel": "ALI_APP",
	"result_id": "121121331",
	"result_code": "0",
	"error_message": "",
