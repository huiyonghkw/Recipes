...
| :-------- | :--------| :-- |:--------|--------:| --------:|
| access_token  | 配件商账户Token | 配件商账户授权Token | string  | 是 | 无 |
| unexpired  | 过期标示 | 1=>未过期 0=>全部 | int | 否 | 0 |
| quoted  | 报价状态| 1=>已报价 2=>未报价 0=>全部 | int | 否 | 0 |
| limit  | 限制大小 | 指定返回记录的数量，系统会做验证    | int | 否 | 50 |
| offset  | 偏移量 | 指定返回记录的开始位置    | int  | 否 | 0 |
...
**请求实例**：
```json
GET http://api.weipei.cc/v1/accessory-shop/purchase-order-lists?access_token=3e16cdaa0061ee89106b9112890a419397b4613s&limit=100&offset=0&status=3
```
**返回结果**：
```json
{
...
| 参数名      |含义   | 规则说明  | 参数类型 | 是否必须 | 缺省值 |
| :-------- | :--------| :-- |:--------|--------:| --------:|
| servicer  | 服务商类型 | 1=>配件商 2=>维修厂   | int  | 是 | 1 |
| province  | 省份 | 省份编号   | int  | 否 | 无 |
| city  | 城市 | 城市编号   | int  | 否 | 无 |
| district  | 区县 | 区县编号   | int  | 否 | 无 |
| keyword  | 关键词 | 可以搜索名称、联系人、联系电话、联系电话   | string  | 否 | 无 |
| limit  | 限制数量 | 默认10条   | int  | 否 | 10 |
| offset  | 偏移量 | 默认0   | int  | 否 | 0 |
...
| full_range_service  | 是否服务全车系 | 0否 1是   | int  | 否 | 无 |
...
| 参数名      |含义   | 规则说明  | 参数类型 | 是否必须 | 缺省值 |
| :-------- | :--------| :-- |:--------|--------:| --------:|
| accessory_shop_id  | 编号 | 配件商编号  | int  | 是 | 无 |
| title  | 名称 | 配件商名称  | string  | 是 | 无 |
| contact  | 联系人 | 配件商主要联系人   | string  | 是 | 无 |
| mobile  | 联系电话 | 配件商主要联系电话号码   | string  | 是 | 无 |
| province  | 省份 | 省份编号   | int  | 否 | 无 |
...
| address  | 详细地址 | 配件商详细地址   | string  | 否 | 无 |
| level  | 维修厂等级 | 等级编号   | int  | 否 | 无 |
| motor_repair_shop_id  | 编号 | 维修厂编号   | int  | 是 | 无 |
...
| limit  | 限制大小 | 指定返回记录的数量，系统会做验证    | int | 否 | 10 |
| offset  | 偏移量 | 指定返回记录的开始位置    | int  | 否 | 0 |
...
| 返回参数 |含义| 参数类型 | 长度 |
|:--|:--|--:|--:|
|motor_repair_shop_level_id|等级编号| int| -|
|motor_repair_shop_level_name|当前返回记录数量| int| -|
